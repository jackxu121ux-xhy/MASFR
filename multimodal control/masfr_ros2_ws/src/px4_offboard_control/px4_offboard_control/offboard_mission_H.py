...
class OffboardControl(Node):
    ...
    def __init__(self) -> None:
        ...
        #    Int16, '/robot_state', self.callbackState, 10)
        self.robotState = 1
        ...
        self.timerMainloop = self.create_timer(0.05, self.mainloop)
    ...
    def mainloop(self):
        ...
        if self.offboard_setpoint_counter <= 60:
            ...
            self.publish_position_setpoint(1.059, -0.8956, -1.4032, 0.0)
        elif self.offboard_setpoint_counter <= 240:
            ...
            self.publish_position_setpoint(2.5430, -0.8461, -0.8150, 0.0)
            #-3.14 + (self.offboard_setpoint_counter - 100) * 1.57 / 50
        else:
            self.flight_termination(1.0)

        if self.offboard_setpoint_counter < 261:
            self.offboard_setpoint_counter += 1
        else:
            msg_state = Int16()
            msg_state.data = 1
            self.robot_state_publisher.publish(msg_state)
            exit(0)
